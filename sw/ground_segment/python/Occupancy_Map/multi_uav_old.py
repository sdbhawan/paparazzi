...
# ==============================================================================
#                 APPEND TO BOTTOM OF multi_uav_old.py
# ==============================================================================

class BeliefMap:
    def __init__(self, w, h, res):
        # --- FIX: Use Global soft_poly bounds ---
        # Instead of assuming 0,0, we get the real world bounds
        minx, miny, maxx, maxy = soft_poly.bounds
        
        # Round to nearest grid resolution to keep things aligned
        self.x_min = np.floor(minx / res) * res
        self.y_min = np.floor(miny / res) * res
        self.x_max = np.ceil(maxx / res) * res
        self.y_max = np.ceil(maxy / res) * res
        
        self.grid_x = np.arange(self.x_min, self.x_max + res, res)
        self.grid_y = np.arange(self.y_min, self.y_max + res, res)
        
        XX, YY = np.meshgrid(self.grid_x, self.grid_y)
        self.grid_points_all = np.column_stack([XX.ravel(), YY.ravel()])
        
        # Mask with the soft_poly defined globally in this file
        self.inside_mask = np.array([soft_poly.contains(Point(p)) for p in self.grid_points_all])
        self.grid_points = self.grid_points_all[self.inside_mask]
        
        # Initialize Uniform Belief
        self.belief = 0.5 * np.ones(len(self.grid_points))
        self.initial_entropy = self.calculate_total_entropy()

    def propagate(self, v_drift, sigma):
        # Slice v_drift[:2] to fix shape mismatch
        self.belief = propagate_belief_vectorized(
            self.belief, 
            self.grid_points, 
            v_drift[:2],   
            dt=1.0, 
    ...
    def calculate_total_entropy(self):
        p = np.clip(self.belief, 1e-9, 1 - 1e-9)
        ent = -p * np.log2(p) - (1 - p) * np.log2(1 - p)
        return np.sum(ent)

    @property
    def entropy(self):
        return self.calculate_total_entropy()

    def get_peak_coords(self):
        if len(self.belief) == 0: return np.array([0,0,0])
        idx = np.argmax(self.belief)
        return self.grid_points[idx]


class UAVAgent:
    def __init__(self, id, start_pos, nominal_alt=40.0):
        self.id = id
        self.state = np.array(start_pos, dtype=float) # [x, y, z]
        self.velocity = np.zeros(3)
        self.mode = "explore"
        self.nominal_alt = nominal_alt
        
        # Tracking State Dictionary
        self.tracker = {
            "active": False,
            "confirmed": False,
            "pos": None,           # Target position
            "time": None,          # Time of detection
            "phase": None,         # FSM Phase
            "lock_start": None,
            "cone_start": None,
            "hover_start": None,
        ...
        self.just_aborted_tracking = False # Metric flag

    def step(self, t, dt, belief_map, v_drift, v_wind, params):
        """
        Executes one control step for this agent.
        Wraps the FSM logic from the original main loop.
        """
        self.just_aborted_tracking = False # Reset flag
        
        # Extract params
        grid_points = belief_map.grid_points
        belief = belief_map.belief
        
        vx_des, vy_des, vz_des = 0.0, 0.0, 0.0
        
        # ==========================
        # FSM LOGIC (Copied & Adapted)
        # ==========================
        ...
        if self.mode == "explore":
            ...
            vx_des, vy_des, vz_des, _, _, _ = plan_velocity_ipp_3D(
                ...
                belief, grid_points, soft_poly,
                constraint_poly=None,
                step_length=params['D_STEP_EXPLORE'],
                fov_angle=theta_FOV, v_max=v_max, n_directions=16,
                altitude_candidates=[30, 50, 70], 
                pred_depth=params['HORIZON_EXPLORE'], 
                E_scale=params['E_SCALE_EXPLORE'],
                lam=params['LAMBDA'],
                buffer=params.get('buffer', 10.0)
            )
            # Altitude Hold
            vz_des = np.clip(self.nominal_alt - self.state[2], -1.0, 1.0)

        elif self.mode == "track" and self.tracker["active"]:
            tr = self.tracker
            ...
            # --- PHASE: INITIAL LOCK ---
                ...
                _, center_R, _ = tracking_region(t, tr["pos"], tr["time"], v_drift)
                
                vec = center_R - self.state[:2]
                dist = np.linalg.norm(vec)
                ...
                if (t - tr["lock_start"]) >= params['LOCK_DURATION']:
                    tr["phase"] = "cone_tracking"
                    tr["cone_start"] = t
            
            # --- PHASE: CONE TRACKING (IPP) ---
            elif tr["phase"] == "cone_tracking":
                region_poly, _, _ = tracking_region(t, tr["pos"], tr["time"], v_drift)
                
                # Check Bounds
                valid_area = soft_poly.intersection(region_poly)
                if valid_area.is_empty:
                    self.abort_tracking()
                    return # Exit step
                ...
                if np.any(in_R_mask):
                    p_vals = belief[in_R_mask]
                    frac_high = np.mean(p_vals > confirm_pconf) # 0.6
                    mean_p = np.mean(p_vals)
                    max_p = np.max(p_vals)
                    
                    # Success Condition (Use params)
                    if (mean_p >= params['CONF_THRESH_MEAN'] or max_p >= params['CONF_THRESH_PEAK']):
                        tr["phase"] = "hover_confirm"
                        tr["hover_start"] = t
                        tr["pos"] = self.state[:2].copy() # Anchor
                        tr["time"] = t
                    
                    # Failure Condition
                    elif (t - tr["cone_start"]) > 30.0: # Timeout
                         self.abort_tracking()
                         return
                
                # Plan IPP inside Cone
                soft_cone = region_poly.buffer(10.0)
                vx_ipp, vy_ipp, vz_ipp, _, _, _ = plan_velocity_ipp_3D(
                    self.state, self.velocity, belief, grid_points, soft_poly,
                    constraint_poly=soft_cone,
                    step_length=params['D_STEP_TRACK'],
                    fov_angle=theta_FOV, v_max=v_max, n_directions=16,
                    altitude_candidates=[self.state[2]],
                    pred_depth=params['HORIZON_TRACK'], 
                    E_scale=params['E_SCALE_TRACK'],
                    lam=params['LAMBDA']
                )
                vx_des = vx_ipp + v_drift[0]*0.8 # Feedforward
                vy_des = vy_ipp + v_drift[1]*0.8
                vz_des = vz_ipp

            # --- PHASE: HOVER CONFIRM ---
            elif tr["phase"] == "hover_confirm":
                # Simple P-Control to anchor
                t_elapsed = t - tr["time"]
                target = tr["pos"] + v_drift * t_elapsed
                
                err = target - self.state[:2]
                dist_err = np.linalg.norm(err)
                
                if dist_err > 0.5:
                    speed = np.clip(dist_err * 1.5, 0.5, v_max)
                    d_unit = err / dist_err
                    vx_des = d_unit[0] * speed
                    vy_des = d_unit[1] * speed
                else:
                    vx_des, vy_des = 0.0, 0.0
                ...
                # Descent
                if self.state[2] > 20.0:
                    vz_des = -1.5
                else:
                    vz_des = 0.0
                
                # Check Confirmation (Simulated "Sensor Fusion" check)
                region_poly, _, _ = tracking_region(t, tr["pos"], tr["time"], v_drift)
                in_R_mask = np.array([region_poly.contains(Point(p)) for p in grid_points])
                if np.any(in_R_mask):
                    max_p = np.max(belief[in_R_mask])
                    if max_p >= params['CONF_THRESH_PEAK']:
                        tr["confirmed"] = True
                        tr["active"] = False
                        self.mode = "explore"
                        # Reset altitude
        ...
        # --- APPLY PHYSICS & UPDATE STATE ---
        # Acceleration Limit
        v_curr = self.velocity[:2]
        v_targ = np.array([vx_des, vy_des])
        delta = v_targ - v_curr
        dist_v = np.linalg.norm(delta)
        max_change = params['ACCEL_MAX'] * dt
        
        if dist_v > max_change:
            delta = delta / dist_v * max_change
            
        self.velocity[:2] = v_curr + delta
        self.velocity[2] = vz_des
        
        # Boundary Check
        next_pos = self.state + self.velocity * dt
        if not soft_poly.contains(Point(next_pos[0], next_pos[1])):
             self.velocity[:2] = 0.0 # Simple stop at wall
        
        self.state += self.velocity * dt

    def abort_tracking(self):
        self.tracker["active"] = False
        self.tracker["phase"] = None
        self.mode = "explore"
        self.just_aborted_tracking = True

# Dummy DriftModel class to satisfy import
class DriftModel:
    pass